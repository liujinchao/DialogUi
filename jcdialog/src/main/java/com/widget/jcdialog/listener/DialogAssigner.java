...
public class DialogAssigner implements Assignable {
    ...
    @Override
    public BuildBean assignTimePick(Context context, int gravity, String addTitle, int tag, OnAddressSelectedListener listener) {
        BuildBean bean = new BuildBean();
        bean.mContext = context;
        bean.dateTitle = addTitle;
        bean.gravity = gravity;
        bean.cancelable = true;
        bean.outsideTouchable = true;
        bean.type = CommonConfig.TYPE_TIMEPICK;
        bean.addressListener = listener;
        bean.tag = tag;
        return bean;
    }
